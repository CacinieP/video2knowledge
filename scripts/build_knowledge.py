...
def build_analysis(host: str, model: str | None, raw_text: str, source: str) -> dict:
    ...
    tasks = [
        ("summary",
         "用中文写3-5句话总结这段视频字幕的核心内容。只输出总结正文，不要前缀。"),
        ("timeline",
         "基于字幕，用markdown无序列表提炼最多12个关键时间节点，每行格式: `- [mm:ss] 事件`。只输出列表。"),
        ("key_points",
         "基于字幕，用markdown无序列表列出最多10个核心知识点，每行一个 `- 要点`。只输出列表。"),
        ("qa",
         "基于字幕，生成5到10组中文问答，每个问题一行 `Q: 问题`，紧跟一行 `A: 答案`。只输出问答。"),
        ("glossary",
         "从字幕中提取最重要的术语，用markdown无序列表 `- 术语` 输出。只输出列表。"),
    ]
